...
pub fn spawn_ball(
    ...
    let team = match right_click {
        true => crate::other_sppd::components::Team::Red,
        false => crate::other_sppd::components::Team::Blue,
    };
    ...
    let _entity = cmds.spawn((
        ...
        OriginalMaterial(original_material.clone()),
        DetectionRange(dtk_range),
        AttackRange(atk_range),
        Life{ hp: life, maxhp: life },
        Attack{ degat: atk },
        team,
        AttackCooldown {
...
fn mark_hover<E: EntityEvent>() -> impl Fn(On<E>, Commands) {
    move |event: On<E>, mut cmds: Commands| {
        cmds.entity(event.event_target()).insert(BeingHovered);
    }
}
...
fn unmark_hover<E: EntityEvent>() -> impl Fn(On<E>, Commands) {
    move |event: On<E>, mut cmds: Commands| {
        cmds.entity(event.event_target()).remove::<BeingHovered>();
    }
}
